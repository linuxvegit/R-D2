 ...
 *
 * This module finds all <input type="text"> elements in the document that have
 * an "data-allowed-chars" attribute. It registers keypress, textInput, and
 * textinput event handlers for any such element to restrict the user's input so
 * that only characters that appear in the value of the attribute may be
 ...
 * message element is hidden. This message id element is intended to offer an
 * explanation to the user of why her keystroke was rejected. It should
 * typically be styled with CSS so that it is initially invisible.
 *
 * Here is sample HTML that uses this module. Zipcode: <input id="zip"
 * type="text" data-allowed-chars="0123456789" data-messageid="zipwarn"> <span
 * id="zipwarn" style="color:red;visibility:hidden">Digits only</span>
 *
 * This module is purely unobtrusive: it does not define any symbols in the
 * global namespace.
 */
whenReady(function() { // Run this function when the document is loaded
	...
	for ( var i = 0; i < inputelts.length; i++) {
		...
		if (elt.addEventListener) {
			...
			elt.addEventListener("textinput", filter, false);
		} else { // textinput not supported versions of IE w/o
					// addEventListener()
	...
	function filter(event) {
		...
		var allowed = target.getAttribute("data-allowed-chars"); // Legal
																	// chars
