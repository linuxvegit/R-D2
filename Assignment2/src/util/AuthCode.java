...
public class AuthCode {
	...
	public AuthCode(int width, int height) {
		this.width = width;
		this.height = height;
	...
	private String drawRandomNum(String... createTypeFlag) {
		// 设置颜色
		...
		if (createTypeFlag.length > 0 && null != createTypeFlag[0]) {
			if (createTypeFlag[0].equals("ch")) {
				// 截取汉字
				return createRandomChar(baseChineseChar);
			} else if (createTypeFlag[0].equals("nl")) {
				// 截取数字和字母的组合
				return createRandomChar(baseNumLetter);
			} else if (createTypeFlag[0].equals("n")) {
				// 截取数字
				return createRandomChar(baseNum);
			} else if (createTypeFlag[0].equals("l")) {
				// 截取字母
				return createRandomChar(baseLetter);
			}
		} else {
			// 默认截取数字和字母的组合
			return createRandomChar(baseNumLetter);
	...
	private String createRandomChar(String baseChar) {
